...
public class FarmaciaDao {
	...
	public static int delete(int id) {
		...
		try {
			...
			int cantidad=FarmaciaDao.getMedicamentoById(id).getStockCantidad();
			
			if (cantidad>1) {
				
				PreparedStatement preparedStatment = connection.prepareStatement("UPDATE MEDICAMENTOS set STOCKCANTIDAD=? where IdMedicamento=?");
				preparedStatment.setInt(1, cantidad-1);
